...
\title{One split at a time is often enough: empirical limits of \texorpdfstring{$k$}{k}-sighted decision-tree induction}

\author[1]{Mathias Valla\corref{cor1}}
\ead{email.to.be.completed@example.com}
\cortext[cor1]{Corresponding author.}
\affiliation[1]{
  organization={Affiliation to be completed},
  country={France}
}
...
\begin{keyword}
Decision trees \sep Random forests \sep Greedy algorithms \sep Lookahead induction \sep Optimal trees \sep PMLB
\end{keyword}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{Fig1_accuracy_time.pdf}
...
\caption{Forest-size grid for pure and mixed $k$-sighted bootstrap forests on the 57-dataset mixed-forest benchmark. Left: mean accuracy versus number of trees. Right: the same curve points versus mean fitting time on a logarithmic scale. Each curve is evaluated at 20, 40, 60, 100, and 200 trees. Legend labels use effective sight $\bar{k}$, the average tree sight depth in the ensemble; the two $\bar{k}=1.10$ curves are distinct mixtures, one with 90\% $k=1$ and 10\% $k=2$ trees and one with 95\% $k=1$ and 5\% $k=3$ trees.}
...
The benchmark datasets are available from PMLB \cite{olson2017pmlb}. The open-source implementation, scripts, tables, and an experimental package for spherical tree and $k$-sighted tree experiments will be available at \url{https://github.com/MathiasValla/non-greedy-decision-trees}.
